...
class OpenSKOS():
    ...
    #THE OPENSKOS API IS NOT WORKING PROPERLY ANYMORE!! ONLY A SIMPLE QUERY WORKS, BUT DOES NOT FIND SOMETHING
    #SIMPLE LIKE AMSTERDAM (OR AMS...)
    ...
    def getConceptDetails(self, uri, delveDeeper = True, format='json'):
        ...
        if url:
            ...
            if content:
                if delveDeeper:
                    return json.dumps({ "%s" % uri : self.__getRelatedConceptInfo(content) })
                else:
                    return content
        return None
        ...
        for key, value in data.iteritems():
            if key in ['broader', 'SemanticRelations', 'broadMatch', 'related']:
                ...
                for concept in value:
                    relatedConcept = json.loads(self.getConceptDetails(concept, False))
    ...
    def __sendSearchRequest(self, params):
        ...
        url_values = urllib.urlencode(params)#FIXME fails on weird characters!
        url = url + '?' + url_values
        print url
        if url:
            headers = {'Accept':'application/json'}
    ...
    def __toAutoCompleteResponseList(self, resultList, lang='nl'):
        results = []
        for item in resultList:
            ...
            if item.has_key('uri'):
                uri = item['uri']
                if item.has_key('prefLabel@%s' % lang):
                    prefLabel = item['prefLabel@%s' % lang]
                ...
                if item.has_key('note@%s' % lang):
                    notes.append(','.join(item['note@%s' % lang]))
                if item.has_key('example@%s' % lang):
                    notes.append(','.join(item['example@%s' % lang]))
                if item.has_key('scopeNote'):
                    notes.append(','.join(item['scopeNote@%s' % lang]))

                """Don't add results if it's a person without a scope note. The rest will be added"""
                if not (inSchemes.find('Persoon') != -1 and not item.has_key('scopeNote@%s' % lang)) :
                    if(correctScheme):
